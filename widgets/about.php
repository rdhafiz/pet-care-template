<div class="about-section">
    <div class="container">
        <div class="content">
            <div class="text-center text-white">
                <div class="title">Title Here</div>
                <div class="desc">Lorem ipsum dolor sit amet, consectetur adipiscing elit. <br> Vivamus lacinia odio vitae vestibulum
                    vestibulum.
                </div>
            </div>
            <div class="about-items">
                <div class="row">
                    <div class="col-lg-4 col-md-6">
                        <div class="item text-center text-white mb-4 mb-lg-0">
                            <div class="img">
                                <img src="./assets/images/about-1.png" class="img-fluid" alt="pet care">
                            </div>
                            <div class="sub-title fw-bold">Pet Care</div>
                            <div class="sub-desc">Lorem ipsum dolor sit amet, consectetur adipiscing elit.</div>
                        </div>
                    </div>
                    <div class="col-lg-4 col-md-6">
                        <div class="item text-center text-white mb-4 mb-lg-0">
                            <div class="img">
                                <img src="./assets/images/about-2.png" class="img-fluid" alt="pet shop">
                            </div>
                            <div class="sub-title fw-bold">Pet Shop</div>
                            <div class="sub-desc">Lorem ipsum dolor sit amet, consectetur adipiscing elit.</div>
                        </div>
                    </div>
                    <div class="col-lg-4 col-md-6">
                        <div class="item text-center text-white mb-4 mb-lg-0">
                            <div class="img">
                                <img src="./assets/images/about-3.png" class="img-fluid" alt="pet help">
                            </div>
                            <div class="sub-title fw-bold">Pet Help</div>
                            <div class="sub-desc">Lorem ipsum dolor sit amet, consectetur adipiscing elit.</div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <div class="left-img">
        <img src="./assets/images/about-shape-left.png" alt="shape-left">
    </div>
    <div class="right-img">
        <img src="./assets/images/about-shape-right.png" alt="shape-right">
    </div>
    <div class="arrow-shape">
        <img src="./assets/images/arrow.png" alt="triangle-shape">
    </div>
</div>
